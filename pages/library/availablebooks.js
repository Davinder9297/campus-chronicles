...
export default function Available() {
  ...
  return (<>
    ...
    <div className='h-screen'>
      ...
      <div className="flex h-[85.3%] overflow-y-hidden bg-gradient-to-r from-amber-900 via-yellow-500 to-amber-900">

        <div className="flex-col filter-side w-[15%]  scrollbar-thin scrollbar-track-slate-700 scrollbar-thumb-slate-600  bg-slate-900 py-2 px-3 text-white overflow-y-auto overflow-x-hidden xsm:hidden ">
          ...
          <div className='flex-col space-y-4 ml-2 pt-3'>
            ...
            <div><button className="bg-gray-400 text-sky-900 rounded-full px-3 py-2 focus:bg-yellow-200 focus:text-amber-800 w-full text- " onClick={showresult} value="All">All Books</button></div>
            <div><button className="bg-gray-400 text-sky-900 rounded-full px-3 py-2 focus:bg-yellow-200 focus:text-amber-800 w-full text- " onClick={showresult} value="Mathematics">Mathematics</button></div>
            <div><button className="bg-gray-400 text-sky-900 rounded-full px-3 py-2 focus:bg-yellow-200 focus:text-amber-800 w-full text-" onClick={showresult} value="Physics">Physics</button></div>
            <div><button className="bg-gray-400 text-sky-900 rounded-full px-3 py-2 focus:bg-yellow-200 focus:text-amber-800 w-full text-" onClick={showresult} value="Python">Python</button></div>
            <div><button className="bg-gray-400 text-sky-900 rounded-full px-3 py-2 focus:bg-yellow-200 focus:text-amber-800 w-full text-" onClick={showresult} value="Java">Java</button></div>
            <div><button className="bg-gray-400 text-sky-900 rounded-full px-3 py-2 focus:bg-yellow-200 focus:text-amber-800 w-full text-" onClick={showresult} value="C">C</button></div>
            <div><button className="bg-gray-400 text-sky-900 rounded-full px-3 py-2 focus:bg-yellow-200 focus:text-amber-800 w-full text-" onClick={showresult} value="DBMS">DBMS</button></div>
            <div><button className="bg-gray-400 text-sky-900 rounded-full px-3 py-2 focus:bg-yellow-200 focus:text-amber-800 w-full text-" onClick={showresult} value="Operating System">OS</button></div>
        ...
        <div className='right-side flex-col w-[85%] xsm:w-full'>
            ...
            <div className="pt-3">
              <div className=" flex w-full flex-wrap items-stretch">
                <input
                  value={searchdata}
                  type="search"
                  className="relative m-0 -mr-px block w-96 min-w-0 flex-auto rounded-l border border-solid border-neutral-300 bg-yellow-100  px-3 py-1.5 text-base font-normal text-neutral-700 outline-none transition duration-300 ease-in-out focus:border-primary focus:text-neutral-700 focus:shadow-te-primary focus:outline-none dark:text-neutral-200 dark:placeholder:text-sky-900 xsm:w-[70%]"
                ...
                />
                <button
                  className="relative z-[2] flex items-center rounded-r bg-yellow-700 px-6 py-2.5 text-xs font-medium uppercase leading-tight text-white shadow-md transition duration-150 ease-in-out hover:bg-amber-600 hover:shadow-lg focus:bg-yellow-600 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-amber-800 active:shadow-lg"
                  type="button"
                  id="button-addon1"
                  data-te-ripple-init
          ...
          <div className='flex-col mx-2 py-12 h-full xsm:px-0 xsm:mx-0 '>
            <div className='text-center text-2xl text-white font-serif xsm:mt-0'> All Books</div>
            <div className='flex justify-center'><img className='w-32' src="/hr.png" alt="" /></div>
            <div className={`${show}  w-full overflow-y-auto scrollbar-thin pb-12  scrollbar-track-transparent scrollbar-thumb-transparent h-full  flex flex-wrap  justify-center pt-2 xsm:pt-0 xsm:pb-0 `}>
              <div className={`${norecord} w-full  flex justify-center items-center shadow-2xl shadow-gray-900 mx-5 my-3 xsm:mx-0 xsm:my-0`}>
                <div className='text-xl text-gray-300 font-serif'>No Record Found</div>
              </div>
              {data.map((da, index) => {
                const { image, bookname, authorname } = da;
                return (<>
                  <div className='flex-col mx-5 my-11 h-60 w-44 xsm:mx-0 xsm:w-[25%] xsm:h-32 xsm:my-16 shadow-lg shadow-purple-400'>
                    <img className='h-full w-full' src={image} alt="" />
                    <div className='text-center font-semibold font-serif text-sm xsm:text-xs text-white'>{bookname} <br />({authorname})</div>
                  </div>
